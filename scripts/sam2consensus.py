...
# sam2consensus.py is under the GPL v.3 license
# Original source: https://github.com/edgardomortiz/sam2consensus
...
recommend to perform indel realignment before for optimal results. The consensus
...
def main():
    ...
    parser.add_argument("-d", "--maxdel", dest="maxdel", type=int, default=150,
    ...
    if filename.endswith(".gz"):
        opener = lambda f: gzip.open(f, "rt", encoding="utf-8", errors="replace")
    else:
        opener = lambda f: open(f, "rt", encoding="utf-8", errors="replace")
    ...
    for refname in sequences:
        ...
        for pos in range(len(coverages[refname])):
            ...
            count_nucs = list(count_nucs.items())
        ...
        if insertions[refname] != []:
            ...
            for pos in sorted(ins_tmp2):
                for col in range(len(ins_tmp2[pos])):
                    ...
                    count_nucs = {}
                    for key, value in ins_tmp2[pos][col].items():
                        if value != 0:
                            count_nucs.setdefault(value,[]).append(key)
